...
!********************************************************************************
subroutine roe_ddt(qcL, qcR, njk, dFdQ)
 ...
 !Output
  ...
  real(p2), dimension(5,5), intent(out) :: dFdQ     ! Inviscid flux Jacobian
 ...
 ! No subscript = Roe average
  ...
  type(derivative_data_type_df5), dimension(5)   :: fL     ! Physical flux evaluated at qcL
  type(derivative_data_type_df5), dimension(5)   :: fR     ! Physical flux evaluated at qcR
 
  type(derivative_data_type_df5) :: RT, oneRT               ! RT = sqrt(rhoR/rhoL)
  type(derivative_data_type_df5) :: rho,u,v,w,H,a,qn       ! Roe-averages
  type(derivative_data_type_df5) :: a2inv
 ...
 !  Left state
      ...
      !  vL = qcL(3)
      !  wL = qcL(4)
      qnL = ddt_dot_product(qcL(2:4),njk,3)
        aL = ddt_sqrt(gamma*qcL(1)/rhoL)
        HL = aL*aL*gmoinv + half*(ddt_dot_product(qcL(2:4),qcL(2:4),3))
 ...
 !  Right state
      ...
      !  vR = qcR(3)/qcR(1)
      !  wR = qcR(4)/qcR(1)
      qnR = ddt_dot_product(qcR(2:4),njk,3)
        aR = ddt_sqrt(gamma*qcR(1)/rhoR)
        HR = aR*aR*gmoinv + half*(ddt_dot_product(qcR(2:4),qcR(2:4),3))
    ...
    RT = ddt_sqrt(rhoR/rhoL)
        oneRT = one / (one + RT)
          rho = RT*rhoL                                        !Roe-averaged density
            ...
            a = ddt_sqrt( (gammamo)*(H-half*(u*u + v*v + w*w)) ) !Roe-averaged speed of sound
 ...
 !Absolute values of the wave Speeds
   ...
   ws(3:4) = ddt_abs(qn)   !Entropy wave
...
! Output
 ...
 subroutine roe_lm_w_ddt(ucL, ucR, njk, ur2L, ur2R, num_flux,dFdU,wsn)
  ...
  !Input
  type(derivative_data_type_df5), dimension(5), intent( in) :: ucL !Left  state in conservative variables.
  type(derivative_data_type_df5), dimension(5), intent( in) :: ucR !Right state in conservative variables.
  ...
  !Output
  real(p2), dimension(5)  , intent(out) :: num_flux ! Inviscid Flux
  real(p2), dimension(5,5), intent(out) :: dFdU     ! Inviscid flux Jacobian
  real(p2),                 intent(out) :: wsn      ! Max wave speed
  ...
  type(derivative_data_type_df5) :: absU, ws2, ws3         ! accoustic wave speeds |u'+c'| and |u'-c'|
  ...
  type(derivative_data_type_df5), dimension(5)   :: ad_num_flux
 ...
 !Primitive and other variables.
  ...
  do i = 1,5
    diss(i) = ddt_dot_product(diss_cartesian(:,i), njk, 3)
  end do

  ! This is the numerical flux: Roe flux = 1/2 *[  Fn(UL)+Fn(UR) - GAMMA|An|(WR-WL) ]
  ad_num_flux = half * (fL + fR - diss)
       
  do i=1,5
    num_flux(i) = ad_num_flux(i)%f
    do j = 1,5
      dFdU(i,j) = ad_num_flux(i)%df(j)
    end do
  end do
  ! Max wave speed normal to the face:
  wsn = abs(uprime%f) + cprime%f
end subroutine roe_lm_w_ddt
!--------------------------------------------------------------------------------
