module sparse_scalar_matrix
    ...
    ! This will allow us to abstract the scalar and scalar operations
    interface sparse_bool_times_sparse_real
        module procedure sparse_bool_times_sparse_real_scalar
    end interface sparse_bool_times_sparse_real

    interface sparse_real_times_sparse_bool
        module procedure sparse_real_times_sparse_bool_scalar
    ...
    subroutine sparse_bool_times_sparse_real_scalar(A_m,AC,AR,BV,BC,BR, prodV,prodC,prodR)
        ! This function performs an efficient computation of A*B=C where A is m*n scalar matrix and B is n*k logical array, and A,B, 
        ...
        integer,  dimension(:), intent(in) :: AR     ! Row indices of A
        ...
        real(p2),dimension(:),  intent(out):: prodV ! Values of C
        integer, dimension(:),  INTENT(OUT):: prodC     ! Column indices of C
        integer, dimension(:),  INTENT(OUT):: prodR          ! Row indices of C
            ...
            do j = AR(i),(AR(i+1)-1)
                ...
                if (j == AR(i)) then
                    if (BR(b_i+1)-BR(b_i) > size(queue1_C)) then
                        ...
                        allocate(queue1_V(BR(b_i+1)-BR(b_i) ))
                    end if
                    queue1_C = 0
                    queue1_V = zero
                    ...
                    if (BR(b_i+1)-BR(b_i) > size(queue2_C)) then
                        ...
                        allocate(queue2_V(BR(b_i+1)-BR(b_i) ))
                    end if
                    queue2_C = 0
                    queue2_V = zero
                    do b_j = BR(b_i),(BR(b_i+1)-1)
                    ...
                    call queue_sort_scalar(queue1_V,queue1_C,size(queue1_C),queue2_V,queue2_C,size(queue2_C))
                end if
            end do
        ...
        if (allocated(queue2_C))  deallocate(queue2_C)
        if (allocated(queue2_V))  deallocate(queue2_V)
    end subroutine sparse_bool_times_sparse_real_scalar

    subroutine sparse_real_times_sparse_bool_scalar(A_m,AV,AC,AR,BC,BR, prodV,prodC,prodR)
        ! This function performs an efficient computation of A*B=C where A is m*n scalar matrix and B is n*k logical array, and A,B, 
        ! and C are all stored using a Compressed Sparse Row Matrix format.  The method is based off the paper 
        ! https://doi.org/10.1109/MICRO50266.2020.00068
        ! 
        ...
        if (allocated(queue2_C))  deallocate(queue2_C)
        if (allocated(queue2_V))  deallocate(queue2_V)
    end subroutine sparse_real_times_sparse_bool_scalar

    subroutine sparseMat_times_vector(ncells,V,C,R,x,b)
        ! This subroutine computes A*x = b where A is a sparse scalar matrix, represented by V, C, and R, and x and b are dense 
        ! scalar vectors (a ncells x 1 vector or nEQ x 1 scalars)
        use common , only : p2, zero
    ...
    end subroutine sparseMat_times_vector

    subroutine queue_sort_scalar(queue1_V,queue1_C,length1,queue2_V,queue2_C,length2)
        ...
        real(p2), dimension(:),          intent(inout) :: queue2_V
        integer,  dimension(:),          intent(inout) :: queue2_C
        ...
        allocate(helperV(length1+length2))
    ...
    end subroutine queue_sort_scalar

end module sparse_scalar_matrix