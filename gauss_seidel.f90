module gauss_seidel
    ...
    interface gauss_seidel_sweep
        module procedure gauss_seidel_sweep_block
        module procedure gauss_seidel_sweep_scalar
    end interface gauss_seidel_sweep
    ...
    subroutine gauss_seidel_sweep_block(nq,ncells,res,V,C,R,Dinv,omega_lrelax,correction, linear_res_norm)
    ...
    end subroutine gauss_seidel_sweep_block

    subroutine gauss_seidel_sweep_scalar(ncells,res,V,C,R,Dinv,omega_lrelax,correction, linear_res_norm)
        ...
        integer , dimension(:),             intent(in)   :: C      ! Column index of A
        integer , dimension(:),             intent(in)   :: R      ! Start index of A
        real(p2), dimension(:),             intent(in)   :: Dinv   ! Inverse of A
        real(p2),                           intent(in)   :: omega_lrelax
        !INOUT
        real(p2), dimension(:),          intent(inout)   :: correction
        !OUTPUT
        real(p2)                   ,     intent(out)     :: linear_res_norm
...
! local
        ...
        gs_loop : do i = 1,ncells ! loop through rows
            ...
            ! ! Update du by the GS relaxation:
                    ...
                    !                     diagonal block        sum of off-diagonal block contributions
                    !       dUj = omega*{ [V/dtj+dR/dUj]^{-1}*(-[dRj/dU1]*dU1 -[dRj/dU2]*dU2 -[dRj/dU3]*dU3 -Res_j) - dUj }
            linear_res = Dinv(i) * b - correction(i)
            correction(i) = correction(i) + omega_lrelax * linear_res
            linear_res_norm = linear_res_norm + abs(linear_res)
        end do gs_loop
        ...
        !---------------------------------------------------------
        linear_res_norm = linear_res_norm / real(ncells, p2)
    end subroutine gauss_seidel_sweep_scalar
